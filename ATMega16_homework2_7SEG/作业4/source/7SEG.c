...
void main()
{
  ...
  while(1)
  {
	...
	PORTD = 0x01;
	PORTC = table_ca[numberA];  //因第一列只可以设置numberA，所以渲染numberA
	delay_ms(10);
	...
	PORTD = 0x01;
	PORTC = table_ca[numberA];    //渲染numberA
	delay_ms(5);
	
	PORTD = 0x02;
	PORTC = table_ca[numberB];    //渲染numberB
	delay_ms(5);
	...
	switch(rule)
	{
	  ...
	  case 4: numberC_2 = (numberA/numberB)%10;		  			       //获得商的个位数
			  ...
			  break; 
	  case 5: PORTD = 0x04;
	  	   	  PORTC = table_ca[numberC_1];    //渲染numberC的十位数
			  delay_ms(10);
			  
			  PORTD = 0x08;
			  PORTC = table_ca[numberC_2];    //渲染numberC的个位数
			  delay_ms(10);
			  break; 
	}
  }
